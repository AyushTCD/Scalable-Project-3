...
void sendMessagesToPeer() // like client
{
	...
	scanf("%d", &PORT_server);
    char remote_peer_ip[INET_ADDRSTRLEN];  // The remote Raspberry Pi's IP address
    printf("Enter the IP address of the remote Raspberry Pi: ");
    scanf("%s", remote_peer_ip);
    ...
    // Specify the IP address of the remote Raspberry Pi
    inet_pton(AF_INET, remote_peer_ip, &(remote_peer_address.sin_addr));
